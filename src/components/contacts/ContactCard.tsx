  ...
  return (
    ...
    >
      <div className="p-4 flex flex-col h-full relative">
        ...
        {/* Info List – one row per attribute with icon */}
        <div className="w-full px-2 mb-4 flex flex-col items-center space-y-2">
            ...
            {contact.job && (
              <div className="flex items-center gap-2 text-sm text-center">
                <Briefcase size={16} className={iconCls} />
                <span className="font-medium">{contact.job}</span>
              </div>
            )}
            */}
            ...
            {(contact.age || contact.gender) && (
              <div className="flex items-center gap-2 text-sm text-center">
                ...
                <span className="font-medium">
                  {[contact.age, contact.gender].filter(Boolean).join(' · ')}
                </span>
              </div>
            )}
            */}
        ...
        <div className="flex items-center justify-center gap-2 lg:gap-0">
            ...
            {contact.instagram && (
              <button 
                onClick={(e) => handleSocialClick(e, 'instagram', contact.instagram)} 
                className="text-[#E4405F] hover:text-[#E4405F] hover:bg-[#E4405F]/10 p-2 rounded-full transition-all hover:scale-110 active:scale-90"
              >
                <Instagram size={18} />
              </button>
            )}
            ...
            {contact.facebook && (
              <button 
                onClick={(e) => handleSocialClick(e, 'facebook', contact.facebook)} 
                className="text-[#1877F2] hover:text-[#1877F2] hover:bg-[#1877F2]/10 p-2 rounded-full transition-all hover:scale-110 active:scale-90"
              >
                <Facebook size={18} />
              </button>
            )}
            ...
            {contact.linkedin && (
              <button 
                onClick={(e) => handleSocialClick(e, 'linkedin', contact.linkedin)} 
                className="text-[#0A66C2] hover:text-[#0A66C2] hover:bg-[#0A66C2]/10 p-2 rounded-full transition-all hover:scale-110 active:scale-90"
              >
                <Linkedin size={18} />
              </button>
            )}
            ...
            {contact.youtube && (
              <button 
                onClick={(e) => handleSocialClick(e, 'youtube', contact.youtube)} 
                className="text-[#FF0000] hover:text-[#FF0000] hover:bg-[#FF0000]/10 p-2 rounded-full transition-all hover:scale-110 active:scale-90"
              >
                <Youtube size={18} />
              </button>
            )}
            ...
            {contact.tiktok && (
              <button 
                onClick={(e) => handleSocialClick(e, 'tiktok', contact.tiktok)} 
                className="text-foreground hover:text-foreground hover:bg-muted p-2 rounded-full transition-all hover:scale-110 active:scale-90"
              >
                <TiktokIcon size={18} />
              </button>
            )}
