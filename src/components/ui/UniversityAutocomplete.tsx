...
export function UniversityAutocomplete({
  ...
  useEffect(() => {
    ...
    debounceTimer.current = setTimeout(() => {
      const results = searchUniversities(value, 8);
  ...
  const handleSelectSuggestion = (university: string) => {
    onChange(university);
    setSuggestions([]);
    setShowSuggestions(false);
    setIsFocused(false);
  ...
  return (
    <div className="space-y-3">
      <div className="space-y-2 relative">
        {label && <Label htmlFor="university">{label}</Label>}
            ...
            {suggestions.map((university, index) => (
              <Badge
                ...
                onMouseDown={(e) => {
                  ...
                  handleSelectSuggestion(university);
                }}
              >
                {university}
              </Badge>
      ...
      {!disabled && isFocused && value.length >= 2 && suggestions.length === 0 && !showSuggestions && (
        <p className="text-xs text-muted-foreground">
          No matching universities found. You can still enter a custom university name.
        </p>
      )}
    </div>
  );
